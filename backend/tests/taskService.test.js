...
describe('taskService', () => {
  ...
  test('getTasks deve retornar uma lista de tarefas', async () => {
    ...
    const mockSnapshot = {
      docs: [
        { id: 'task-id-1', data: () => ({ description: 'Task 1', status: 'todo' }) },
        { id: 'task-id-2', data: () => ({ description: 'Task 2', status: 'done' }) },
      ],
    };
  ...
  test('updateTaskStatus deve atualizar o status de uma tarefa', async () => {
    ...
    expect(db.collection().doc().update).toHaveBeenCalledWith({ status: newStatus }); // Verifica se o status foi atualizado
  });
});
